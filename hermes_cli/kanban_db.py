...
CREATE TABLE IF NOT EXISTS kanban_goals (
    ...
    verify_promote_adapter_id TEXT,
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if goals_table_exists:
        ...
        additive_goal_columns = {
            "workflow_version": "workflow_version INTEGER NOT NULL DEFAULT 1",
            "task_contract_ref": "task_contract_ref TEXT",
            "task_contract_json": "task_contract_json TEXT",
            "task_contract_hash": "task_contract_hash TEXT",
            "resolver_id": "resolver_id TEXT",
            "orchestrator_profile": "orchestrator_profile TEXT",
            "verify_promote_adapter_id": "verify_promote_adapter_id TEXT",
            "promotion_evidence": "promotion_evidence TEXT",
        }
        goal_cols = {
            row["name"] for row in conn.execute("PRAGMA table_info(kanban_goals)")
        }
        for column, ddl in additive_goal_columns.items():
            if column not in goal_cols:
                _add_column_if_missing(conn, "kanban_goals", column, ddl)
        conn.execute(
            """
            CREATE TRIGGER IF NOT EXISTS kanban_goal_v2_contract_immutable
            BEFORE UPDATE OF
                objective, workflow_version,
                task_contract_ref, task_contract_json, task_contract_hash,
                resolver_id, orchestrator_profile, verify_promote_adapter_id,
                builder_profile, reviewer_profile, reviewer_skill_digest
            ON kanban_goals
            WHEN OLD.workflow_version >= 2 AND (
                ...
                OR NEW.task_contract_json IS NOT OLD.task_contract_json
                OR NEW.task_contract_hash IS NOT OLD.task_contract_hash
                OR NEW.resolver_id IS NOT OLD.resolver_id
                OR NEW.orchestrator_profile IS NOT OLD.orchestrator_profile
                OR NEW.verify_promote_adapter_id
                    IS NOT OLD.verify_promote_adapter_id
                OR NEW.builder_profile IS NOT OLD.builder_profile
                OR NEW.reviewer_profile IS NOT OLD.reviewer_profile
                OR NEW.reviewer_skill_digest IS NOT OLD.reviewer_skill_digest
            )
            BEGIN
                SELECT RAISE(ABORT, 'Workflow V2 task contract binding is immutable');
            END
            """
        )

    goal_outbox_table_exists = (
        conn.execute(
...
def create_trusted_durable_goal_record(
    conn: sqlite3.Connection,
    *,
    ...
    """Atomically persist a Workflow V2 contract and its initial PLAN task."""
    from hermes_cli.kanban_trusted_stages import TaskContract

    contract = TaskContract.from_payload(dict(task_contract)).as_payload()
    objective = str(contract.get("objective") or "").strip()
    contract_ref = str(contract.get("reference") or "").strip()
    contract_hash = str(contract.get("contract_hash") or "").strip()
    base_revision = str(contract.get("base_revision") or "").strip()
    scope = contract.get("scope")
    gates = contract.get("gates")
    if not objective or not contract_ref or not contract_hash or not base_revision:
        raise ValueError("trusted task contract binding is incomplete")
    if (
        ...
        raise ValueError("trusted task contract scope and gates are required")
    required_ids = {
        "resolver_id": str(resolver_id or "").strip(),
        "verify_promote_adapter_id": str(verify_promote_adapter_id or "").strip(),
        "orchestrator_profile": str(orchestrator_profile or "").strip(),
        "builder_profile": str(builder_profile or "").strip(),
        "reviewer_profile": str(reviewer_profile or "").strip(),
    }
    for name, value in required_ids.items():
        if not value:
            raise ValueError(f"{name} is required")
    reviewer_digest = str(reviewer_skill_digest or "").strip() or None
    if reviewer_digest is not None and not re.fullmatch(
        r"[0-9a-f]{64}", reviewer_digest
    ):
        raise ValueError("reviewer_skill_digest must be lowercase 64-hex")
    try:
        repair_limit = int(repair_budget)
        ...
        workflow = int(workflow_version)
    except (TypeError, ValueError) as exc:
        raise ValueError("durable goal versions and budgets must be integers") from exc
    if repair_limit < 0 or review_limit < 0:
        raise ValueError("durable goal budgets must be non-negative")
    if workflow != 2 or schema < 2 or protocol < 2:
        raise ValueError("trusted durable goals require Workflow V2")
    platform = str(origin.get("platform") or "").strip().lower()
    chat_id = str(origin.get("chat_id") or "").strip()
    if not platform or not chat_id:
        raise ValueError("durable goals require a gateway platform and chat_id")
    ...
    if message_id:
        existing = conn.execute(
            "SELECT * FROM kanban_goals "
            "WHERE origin_platform = ? AND origin_chat_id = ? "
            "AND origin_message_id = ?",
            (platform, chat_id, message_id),
        ).fetchone()
        if existing is not None:
            if int(existing["workflow_version"] or 1) != 2:
                raise ValueError(
                    "origin message is already bound to legacy workflow history"
                )
            expected_binding = {
                "board_slug": str(board_slug or "").strip(),
                "task_contract_ref": contract_ref,
                "task_contract_hash": contract_hash,
                "resolver_id": required_ids["resolver_id"],
                "verify_promote_adapter_id": required_ids["verify_promote_adapter_id"],
                "orchestrator_profile": required_ids["orchestrator_profile"],
                "builder_profile": required_ids["builder_profile"],
                "reviewer_profile": required_ids["reviewer_profile"],
                "reviewer_skill_digest": reviewer_digest,
                "repair_budget": repair_limit,
                "review_retry_budget": review_limit,
            }
            if any(
                ...
                raise ValueError(
                    "origin message is already bound to a different "
                    "Workflow V2 contract"
                )
            binding = conn.execute(
                "SELECT task_id FROM kanban_goal_tasks "
                "WHERE goal_id = ? AND stage = 'PLAN' AND attempt = 0",
                (existing["id"],),
            ).fetchone()
    ...
    plan_body = (
        f"Durable goal contract: {contract_ref}\n\n"
        f"Stage: PLAN. Contract hash: {contract_hash}. "
        f"Base revision: {base_revision}. Scope and gates are immutable. "
        "Return PLAN_ACCEPTED or HUMAN_GATE through structured durable-goal "
        "metadata bound to this contract. Do not merge, push, or deploy."
    )
    with write_txn(conn):
        conn.execute(
            """
            INSERT INTO kanban_goals (
                id, objective, status, current_stage, workflow_version,
                schema_version, protocol_version, board_slug,
                origin_platform, origin_chat_id, origin_chat_type,
                origin_thread_id, origin_user_id, origin_message_id,
                notifier_profile, delivery_metadata,
                ...
                resolver_id, orchestrator_profile, verify_promote_adapter_id,
                builder_profile, verifier_profile, reviewer_profile,
                reviewer_skill_digest, repair_budget, review_retry_budget,
            ...
            (
                ...
                contract_json,
                contract_hash,
                required_ids["resolver_id"],
                required_ids["orchestrator_profile"],
                required_ids["verify_promote_adapter_id"],
                required_ids["builder_profile"],
                required_ids["reviewer_profile"],
                reviewer_digest,
                repair_limit,
                review_limit,
                now,
        ...
        conn.execute(
            ...
            INSERT INTO tasks (
                ...
                created_by, created_at, workspace_kind, idempotency_key,
                current_step_key
            ) VALUES (?, ?, ?, ?, 'durable_ready', 0, ?, ?, 'worktree', ?, 'PLAN')
            """,
            (
                task_id,
                f"[Durable PLAN] {objective}"[:240],
                plan_body,
                required_ids["orchestrator_profile"],
                f"durable-goal:{goal_id}",
                now,
                f"durable-goal:{goal_id}:PLAN:0",
            ),
        )
        _append_event(
            conn,
            ...
            "created",
            {
                "assignee": required_ids["orchestrator_profile"],
                "status": "durable_ready",
                "durable_goal_id": goal_id,
                "stage": "PLAN",
            },
...
def list_durable_goal_task_rows(
    ...
    return conn.execute(
        "SELECT * FROM kanban_goal_tasks WHERE goal_id = ? ORDER BY created_at, rowid",
        (goal_id,),
    ).fetchall()
...
def _durable_completion_run_is_current(
    ...
    *,
    ...
    try:
        current_run_id = int(current_run_raw) if current_run_raw is not None else None
    except (TypeError, ValueError):
        return False
    return (
    ...
    with write_txn(conn):
        ...
        if reserve_budget == "repair":
            if int(goal["repair_attempts_reserved"]) >= int(goal["repair_budget"]):
                return None
            reservation_sql = "repair_attempts_reserved = repair_attempts_reserved + 1,"
        elif reserve_budget == "review":
            if int(goal["review_attempts_reserved"]) >= int(
                goal["review_retry_budget"]
            ):
                return None
            reservation_sql = "review_attempts_reserved = review_attempts_reserved + 1,"
        else:
            reservation_sql = ""
        ...
        body = (
            ...
            f"Stage: {next_stage}. "
            + (f"Expected candidate SHA: {candidate_sha}. " if candidate_sha else "")
...
def transition_durable_goal_to_waiting_stage(
    conn: sqlite3.Connection,
    *,
    goal_id: str,
    expected_state_version: int,
    ...
    """Consume one current task result and enter a taskless deterministic stage."""
    try:
        state_version = int(expected_state_version)
        event_id = int(completion_event_id)
        run_id = int(expected_run_id)
        ...
        return False
    waiting_stage = str(waiting_stage or "").strip()
    if state_version < 1 or event_id < 1 or run_id < 1 or not waiting_stage:
        return False
    now = int(time.time())
    with write_txn(conn):
        goal = conn.execute(
        ...
        if predecessor is None or predecessor["completion_event_id"] is not None:
            return False
        event = conn.execute(
        ...
        if (
            ...
            or event["run_id"] is None
            or int(event["run_id"]) != run_id
            or not _durable_completion_run_is_current(
                conn, task_id=predecessor_task_id, run_id=run_id
            )
        ):
            return False
        binding_update = conn.execute(
            ...
            (
                ...
                event_id,
                json.dumps(
                    dict(completion_payload), sort_keys=True, separators=(",", ":")
                ),
                goal_id,
                predecessor_task_id,
            ),
        ...
        goal_update = conn.execute(
            ...
            "state_version = state_version + 1, updated_at = ? "
            "WHERE id = ? AND status = 'ACTIVE' AND state_version = ?",
            (waiting_stage, candidate_sha, now, goal_id, state_version),
        )
        if goal_update.rowcount != 1:
...
def transition_durable_goal_from_waiting_to_successor(
    ...
    *,
    ...
    This consumes no adapter and performs no operation.  The caller supplies
    already-produced evidence; this primitive only validates current state and
    records the next task atomically.
    """
    if reserve_budget not in {None, "repair"}:
        raise ValueError("waiting-stage reserve_budget must be 'repair' or None")
    try:
        state_version = int(expected_state_version)
        attempt = int(next_attempt)
    except (TypeError, ValueError):
        return None
    waiting_stage = str(expected_waiting_stage or "").strip()
    next_stage = str(next_stage or "").strip()
    assignee = _canonical_assignee(assignee)
    ...
    if (
        state_version < 1
        or attempt < 0
        or not waiting_stage
        or not next_stage
        or not assignee
        or task_status not in {"ready", "review"}
    ):
        return None
    stored_status = "durable_review" if task_status == "review" else "durable_ready"
    skills_list = [str(skill).strip() for skill in (skills or ()) if str(skill).strip()]
    now = int(time.time())
    successor_id = _new_task_id()
    with write_txn(conn):
        ...
        source_binding = conn.execute(
            "SELECT * FROM kanban_goal_tasks WHERE goal_id = ? AND task_id = ?",
            (goal_id, source_task_id),
        ).fetchone()
        if source_binding is None or source_binding["completion_event_id"] is None:
            return None
        if reserve_budget == "repair":
            if int(goal["repair_attempts_reserved"]) >= int(goal["repair_budget"]):
                return None
        ...
        source_task = conn.execute(
            "SELECT * FROM tasks WHERE id = ?", (source_task_id,)
        ).fetchone()
        if source_task is None:
            return None
        body = (
            f"Durable goal: {goal['objective']}\n\n"
            f"Stage: {next_stage}. Contract hash: {goal['task_contract_hash']}. "
            f"Current candidate SHA: {candidate_sha}. Continue only within the "
            "immutable task contract. Do not merge, push, or deploy. Report "
            "structured completion through metadata.durable_goal."
        )
        conn.execute(
            """
            INSERT INTO tasks (
            ...
            (
                ...
                source_task["tenant"],
                f"durable-goal:{goal_id}:{next_stage}:{attempt}",
                source_task["max_runtime_seconds"],
                source_task["max_retries"],
                next_stage,
                json.dumps(skills_list) if skills_list else None,
            ),
        )
        ...
        conn.execute(
            "INSERT INTO kanban_goal_tasks "
            "(goal_id, task_id, stage, attempt, expected_candidate_sha, created_at) "
            "VALUES (?, ?, ?, ?, ?, ?)",
            (goal_id, successor_id, next_stage, attempt, candidate_sha, now),
        )
        updated = conn.execute(
            f"UPDATE kanban_goals SET {reservation_sql} current_stage = ?, "
            "promotion_evidence = ?, state_version = state_version + 1, updated_at = ? "
            "WHERE id = ? AND status = 'ACTIVE' AND current_stage = ? "
            "AND state_version = ?",
            (
                next_stage,
        ...
        if updated.rowcount != 1:
            raise RuntimeError("durable injected-result state CAS failed")
    return successor_id


def transition_durable_goal_waiting_to_terminal(
    conn: sqlite3.Connection,
    *,
    goal_id: str,
    expected_state_version: int,
    ...
    with write_txn(conn):
        ...
        updated = conn.execute(
            ...
            "blocked_reason = ?, promotion_evidence = ?, "
            "state_version = state_version + 1, updated_at = ? "
            "WHERE id = ? AND status = 'ACTIVE' AND current_stage = ? "
            "AND state_version = ?",
            (
        ...
        conn.execute(
            ...
            (
                ...
                json.dumps(
                    dict(notification_payload), sort_keys=True, separators=(",", ":")
                ),
...
def list_durable_goal_notification_rows(
    ...
    return conn.execute(
        "SELECT * FROM kanban_goal_notification_outbox WHERE goal_id = ? ORDER BY id",
        (goal_id,),
    ).fetchall()
...
def clear_durable_goal_runtime(conn: sqlite3.Connection, *, runtime_id: str) -> bool:
...
def block_legacy_durable_goal_under_v2(
    conn: sqlite3.Connection,
    *,
    goal_id: str,
    expected_state_version: int,
    ...
    """Atomically stop an active V1 durable goal observed by a V2 supervisor."""
    reason = str(reason or "").strip()
    if not reason:
        return False
    now = int(time.time())
    with write_txn(conn):
        ...
        if (
            goal is None
            or goal["status"] != "ACTIVE"
            or int(goal["workflow_version"] or 1) >= 2
            or int(goal["state_version"]) != int(expected_state_version)
        ):
            return False
        updated = conn.execute(
            ...
            "state_version = state_version + 1, updated_at = ? "
            "WHERE id = ? AND status = 'ACTIVE' AND state_version = ? "
            "AND COALESCE(workflow_version, 1) < 2",
            (reason, now, goal_id, int(expected_state_version)),
        )
        if updated.rowcount != 1:
            return False
        ...
        conn.execute(
            ...
            (
                goal_id,
                f"durable-goal:{goal_id}:V2_LEGACY_HUMAN_GATE",
                json.dumps(payload, sort_keys=True, separators=(",", ":")),
                goal["origin_platform"],
                goal["origin_chat_id"],
                goal["origin_chat_type"],
...
def claim_review_task(
    ...
    *,
    ...
    expires = now + _resolve_claim_ttl_seconds(ttl_seconds)
    claimable_statuses = (
        "('review', 'durable_review')" if allow_durable else "('review')"
    )
    with write_txn(conn):
        cur = conn.execute(
            f"""
...
def complete_task(
    ...
    *,
        ...
        with write_txn(conn):
            ...
            if phantom_cards:
                _append_event(
                    ...
                    run_id=(guarded_run_id if expected_run_id is not None else None),
                )
    ...
    dispatch_role = (
        metadata.get("_kanban_dispatch_role") if isinstance(metadata, dict) else None
    )
...
def heartbeat_worker(
    ...
    *,
    ...
    now = int(time.time())
    expires = now + _resolve_claim_ttl_seconds(ttl_seconds) if extend_claim else None
...
def _default_spawn(
    ...
    *,
    ...
    # Role provenance is dispatcher-owned. Never inherit a gateway/session role;
    # resolve it only from the selected profile or the task's forced review role.
