...
DEFAULT_CONFIG = {
    ...
    "kanban": {
        ...
        # Generic, contract-bound durable-goal Workflow V2. These empty IDs
        # keep the feature fail-closed until an operator selects implementations
        # from the trusted in-process registry. V2 has exactly three LLM
        # authorities; verifier_profile is deliberately not a valid V2 key.
        "durable_goals": {
            "workflow_version": 2,
            ...
            "builder_profile": "",
            "reviewer_profile": "",
            "task_contract_resolver": "",
            "verify_promote_adapter": "",
...
def parse_durable_goals_v2_config(config: Dict[str, Any]) -> Dict[str, Any]:
    """Return normalized generic Workflow V2 config or fail closed.

    This parser performs no registry lookup and changes no config. Callers use
    the returned trusted IDs to select already-registered implementations.
    """
    if not isinstance(config, dict):
        raise ValueError("configuration must be a mapping")
    kanban = config.get("kanban")
    durable = kanban.get("durable_goals") if isinstance(kanban, dict) else None
    if not isinstance(durable, dict):
        raise ValueError("configure kanban.durable_goals for Workflow V2")
    if "verifier_profile" in durable:
        raise ValueError(
            "kanban.durable_goals.verifier_profile is not valid for Workflow V2 "
            "(legacy storage is non-authoritative)"
        )
    try:
        workflow_version = int(durable.get("workflow_version"))
    except (TypeError, ValueError) as exc:
        raise ValueError("kanban.durable_goals.workflow_version must be 2") from exc
    if workflow_version != 2:
        raise ValueError("kanban.durable_goals.workflow_version must be 2")
    if str(durable.get("start_mode") or "").strip() != "trusted_contract":
        raise ValueError("kanban.durable_goals.start_mode must be trusted_contract")
    required = {
        "orchestrator_profile": str(durable.get("orchestrator_profile") or "").strip(),
        "builder_profile": str(durable.get("builder_profile") or "").strip(),
        "reviewer_profile": str(durable.get("reviewer_profile") or "").strip(),
        "task_contract_resolver": str(
            durable.get("task_contract_resolver") or ""
        ).strip(),
        "verify_promote_adapter": str(
            durable.get("verify_promote_adapter") or ""
        ).strip(),
        "reviewer_skill_digest": str(
            durable.get("reviewer_skill_digest") or ""
        ).strip(),
    }
    missing = [name for name, value in required.items() if not value]
    if missing:
        raise ValueError(
            "configure " + ", ".join(f"kanban.durable_goals.{name}" for name in missing)
        )
    if not re.fullmatch(r"[0-9a-f]{64}", required["reviewer_skill_digest"]):
        raise ValueError(
            "kanban.durable_goals.reviewer_skill_digest must be lowercase 64-hex"
        )
    try:
        repair_budget = int(durable.get("repair_budget", 1))
        review_retry_budget = int(durable.get("review_retry_budget", 1))
    except (TypeError, ValueError) as exc:
        raise ValueError("durable goal budgets must be integers") from exc
    if repair_budget < 0 or review_retry_budget < 0:
        raise ValueError("durable goal budgets must be non-negative")
    return {
        "workflow_version": workflow_version,
        "start_mode": "trusted_contract",
        "board": str(durable.get("board") or "").strip() or None,
        **required,
