...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_context_command(self, event: MessageEvent) -> str:
        ...
        session_key = self._session_key_for_source(source)
        session_entry = await self.async_session_store.get_or_create_session(source)
        expanded = event.get_command_args().strip().lower() in {
            "all",
            "full",
            "details",
        }
        ...
        if used > 0 and context_length > 0:
            ...
            if ctx is not None:
                ...
                lines.append(
                    t("gateway.context.total_billed", total=f"{total_tokens:,}")
                )
    ...
    async def _handle_agents_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            delegations = [
                d
                for d in list_async_delegations()
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        if getattr(getattr(self, "config", None), "multiplex_profiles", False):
            _command_profile_home = getattr(self, "_resolve_profile_home_for_source")(
                source
            )
        ...
        async def _finish_switch() -> str:
            ...
            ctx = resolve_display_context_length(
                ...
                configured_model=(
                    _sw2_model_cfg.get("default") or _sw2_model_cfg.get("model")
                ),
    ...
    async def _handle_goal_command(self, event: "MessageEvent") -> str:
        ...
        if lower == "durable" or lower.startswith("durable "):
            ...
            if not objective:
                return "Usage: /goal durable contract <opaque-ref>"
            source = event.source
            ...
            if durable_parts and durable_parts[0].lower() == "complete":
                if len(durable_parts) != 3:
                    return "Usage: /goal durable complete <goal-id> <candidate-sha>"
            ...
            try:
                workflow_version = int(durable_cfg.get("workflow_version", 1))
            except (TypeError, ValueError):
                return "Durable goal unavailable: workflow_version must be 1 or 2."
            if workflow_version == 2:
                from hermes_cli.config import parse_durable_goals_v2_config

                try:
                    durable_cfg = parse_durable_goals_v2_config(config)
                except ValueError as exc:
                    return f"Durable goal unavailable: {exc}"
                board = durable_cfg["board"]
                if len(durable_parts) != 2 or durable_parts[0].lower() != "contract":
                    return "Usage: /goal durable contract <opaque-ref>"
                required_profiles = {
                    "orchestrator_profile": str(
                        durable_cfg.get("orchestrator_profile") or ""
                    ).strip(),
                    "builder_profile": str(
                        durable_cfg.get("builder_profile") or ""
                    ).strip(),
                    "reviewer_profile": str(
                        durable_cfg.get("reviewer_profile") or ""
                    ).strip(),
                }
                missing = [
                    name for name, value in required_profiles.items() if not value
                ]
                if missing:
                    return (
                ...
                resolver_id = str(
                    durable_cfg.get("task_contract_resolver") or ""
                ).strip()
                adapter_id = str(
                    durable_cfg.get("verify_promote_adapter") or ""
                ).strip()
                if not resolver_id or not adapter_id:
                    return (
                        "Durable goal unavailable: configure "
                        "kanban.durable_goals.task_contract_resolver and "
                        "kanban.durable_goals.verify_promote_adapter."
                    )
                registry = getattr(self, "_trusted_stage_registry", None) or getattr(
                    self, "trusted_stage_registry", None
                )
                if registry is None:
                    ...
                    registry = get_trusted_stage_registry()
                try:
                    resolver = registry.resolver(resolver_id)
                    registry.adapter(adapter_id)
                    contract = resolver.resolve(durable_parts[1])
                ...
                try:
                    ...
                    review_retry_budget = int(durable_cfg.get("review_retry_budget", 1))
                except (TypeError, ValueError):
                ...
                def _create_trusted_goal():
                    ...
                    with _kb.connect_closing(board=resolved_board) as conn:
                        ...
                        return create_trusted_durable_goal(
                            ...
                            verify_promote_adapter_id=adapter_id,
                            orchestrator_profile=required_profiles[
                ...
                try:
                    created = await asyncio.to_thread(_create_trusted_goal)
                except ValueError as exc:
                    return f"Durable goal unavailable: {exc}"
                except Exception:
                    logger.exception("durable trusted goal creation failed")
                    return (
                        "Durable goal unavailable due to an internal error. "
                        "No goal was created."
                    )
                return (
                    f"Durable goal created {created.goal_id} (PLAN task "
                    f"{created.task_id}). Progress is supervised by Kanban; "
                    "no chat continuation was queued."
                )
            return (
                "Durable goal unavailable: Workflow V1 is retired and cannot "
                "start new work; existing history remains readable."
            )
    ...
    async def _handle_set_home_command(self, event: MessageEvent) -> str:
        ...
        home = HomeChannel(
            ...
            scope_id=(
                str(source.scope_id) if getattr(source, "scope_id", None) else None
            ),
        )
    ...
    @staticmethod
    def _fenced_truncated_diff(
        diff: str, max_lines: int = 60, max_chars: int = 3000
    ) -> str:
    ...
    def _reasoning_picker_choices(self, current_effort: str) -> list:
        ...
        choices.extend([
            {
                "value": "reset",
                "label": t("gateway.reasoning.choice_reset"),
                "is_current": False,
            },
            {
                "value": "show",
                "label": t("gateway.reasoning.choice_show"),
                "is_current": False,
            },
            {
                "value": "hide",
                "label": t("gateway.reasoning.choice_hide"),
                "is_current": False,
            },
        ])
        return choices
    ...
    async def _handle_compress_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            try:
                ...
                _lock_skipped = getattr(
                    tmp_agent, "_compression_skipped_due_to_lock", None
                )
